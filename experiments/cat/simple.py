...
def cosine_variance_attention(vec, memory, **kwargs):
    sim = cosine_similarity(vec, memory)
    sim_var = sim.var(1)
    sum_sim_var = sim_var.sum()
    if sum_sim_var == 0: return np.ones((1, len(vec))) / len(vec)
    return (sim_var / sum_sim_var)[None, :]
